...
for i in files:
    ...
    innings = len(ini)
    shrink = random.randint(30,60)

    for k in range(int((shrink/100)*innings)):
            del ag[-1]
            del sc[-1]
            del years[-1]
            del ini[-1]
    ...
    if(sum(sc) >= 200):
        add_data("data_innings.csv", [innings, ret_age, deb_age, len(ini), sum(sc), fiftees, in3])
    # print([innings, ret_age, deb_age, len(ini), sum(sc), fiftees, in3])
    id += 1
